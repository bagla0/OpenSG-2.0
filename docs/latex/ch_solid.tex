...
\midrule
1 & beam & \texttt{[eps11 gam12 gam13 k1 k2 k3]} & \texttt{Timoshenko} \\
2 & plate & \texttt{[N11 N22 N12 M11 M22 M12]} & \texttt{Classical Plate} \\
3 & 3-D elastic & Voigt \texttt{[11 22 33 23 13 12]} & (none) \\
\bottomrule
\end{tabular}
\caption{\texttt{n\_model} on \texttt{plate\_homo\_2d}. The infix goes into
\texttt{The Effective <infix> Stiffness Matrix}; the plate route becomes
\texttt{Reissner-Mindlin Plate} when \texttt{shear\_refined=True} produces an
$8\times8$.}
...
matrix whose title follows \texttt{model} --- \texttt{Reissner-Mindlin Plate}
...
\captree{
  ...
  \node[funcnode, below=of i1] (f1) {\texttt{sg\_homo.plate\_homo\_2d}\\
    ...
    \texttt{dim}, \texttt{nodes}, \texttt{cells},
    \texttt{mat\_id}, \texttt{scale}};
...
section: \texttt{dim: 2}, \texttt{scale: 35.248}, 4251 nodes, 6640 three-node
triangles, three materials. Coordinates are in millimetres and moduli in MPa.

The \texttt{User Input} block of \texttt{plate\_homo\_2dsg.py} is three items:

\begin{lstlisting}[style=opensg,caption={The User Input block of plate\_homo\_2dsg.py.}]
...
\emph{overrides} the material blocks stored in the SG file. That override is the
intended route here and it is not cosmetic: opening
\texttt{RHC\_SW\_2UC\_45.yaml} shows materials 1 and 2 stored as
\texttt{type: 2}, that is as full $6\times6$ $\mathbf{C}$ blocks already rotated
to $\pm45^\circ$ (their $C_{16}$ entries are $\mp25368.7$), while material 3 is
\texttt{type: 0}, isotropic aluminium with $E = 69000$~MPa and $\nu = 0.3$.
Supplying \texttt{angles} together with the stored blocks would rotate the plies
twice. Rebuilding from engineering constants and rotating in code is
unambiguous. The same numbers are recorded next to the runner in
\texttt{mat\_override.yaml} for reference; omit both arguments to use the file's
...
block, so
\texttt{python -m opensg\_solid.rm\_plate\_1D.helper.sc\_to\_yaml file.sc}
does nothing. Call
...
material. The script also prints \texttt{r["C\_eff"]}, the same $6\times6$ it
wrote. The \texttt{.out} banner records the SG measure and the boundary
treatment:
...
against the mesh: the nodes run from $y_1 = -17.624$ to $+17.624$~mm. The
...
Passing \texttt{shear\_refined=True} to \texttt{plate\_homo\_2d} additionally
...
\texttt{Reissner-Mindlin Plate} instead of \texttt{Classical Plate}; otherwise
the run falls back to the $6\times6$. The argument applies to
\texttt{n\_model=2} only and raises for any other model.
...
structure gene (\texttt{dim: 1}, two-node line elements through a stack), which
runs through the identical call by changing \texttt{name}.
...
\texttt{plate\_dehom\_2dsg.py} takes the same \texttt{name},
\texttt{material\_param} and \texttt{angles} as Section~\ref{sec:solid-plate2d},
plus one new line --- the macro plate strain, in the order
\texttt{[e11, e22, 2e12, k11, k22, 2k12]}:

\begin{lstlisting}[style=opensg,caption={The macro plate strain driving the recovery.}]
...
\texttt{dim}/\texttt{nodes}/\texttt{cells}/\texttt{mat\_id}/\texttt{materials}
...
\captree{
  ...
  \node[funcnode, below=of f4] (f5) {\texttt{sg\_homo.write\_sc\_K}, then the\\
    runner scales by \texttt{omega} and reads\\ \texttt{Sample\_N.sc.k}
    term by term};
...
\subsection{Normalization: per unit material volume, or per unit cell}

The \texttt{.out} that \texttt{plate\_homo\_2d} writes is in the SG measure:
$\omega$ appears in the banner, and for these samples $\omega$ \emph{is} the
relative density, because the bounding cell has unit volume. So
\texttt{Sample\_1\_periodic.out} opens with
\texttt{ OpenSG msg-solid 3D elastic model, omega 0.3, periodic} and reports
$C_{11} = 3.3967192\times10^{10}$~Pa --- the stiffness per unit volume of
\emph{aluminium}. The sample runners multiply by $\omega$ to get the per-unit-cell
value the vendor prints, $3.3967192\times10^{10}\times0.3 = 1.019016\times10^{10}$~Pa
$= 10190.2$~MPa. Every comparison table below is per unit cell, in MPa.
...
prints for Sample 2 --- $E_1 = 1.1383725\times10^{10}$~Pa,
$G_{12} = 1.2914214\times10^{10}$~Pa, $\nu_{12} = 0.41174$ --- are per unit
material volume in the same way.
...
containing \texttt{The Effective Timoshenko Stiffness Matrix} followed by
\texttt{The Effective Timoshenko Compliance Matrix}, with banner
...
the \texttt{name} variable of the runner, \texttt{<ff stem>} the basename of the
\texttt{.ff} table.}
...
3 & \texttt{name}, \texttt{material\_param}, \texttt{angles};
    \texttt{python plate\_homo\_2dsg.py}
  & \texttt{<name>.out}, \texttt{<name>\_mesh.png} \\
4 & the above plus \texttt{epsilon\_bar};
    \texttt{python plate\_dehom\_2dsg.py}
  & \texttt{<name>.out}, \texttt{<name>\_plate\_ABD.out},
...
7 & \texttt{name}, \texttt{material\_param}, \texttt{angles};
    \texttt{python beam\_homo\_sg.py}
  & \texttt{<name>.out}, \texttt{<name>\_mesh.png} \\
8 & the above plus \texttt{epsilon\_bar};
    \texttt{python beam\_dehom\_sg.py}
  & \texttt{<name>.out}, \texttt{<name>\_beam\_Timo.out},
    \texttt{<name>\_dehom.txt}, \texttt{.vtk}, \texttt{.SM}, \texttt{.EM},
    \texttt{.U}, \texttt{<name>\_mesh.png} \\
\end{longtable}
\endgroup
