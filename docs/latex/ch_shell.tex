...
The whole capability is the chain below: one YAML in, one ring solve, four
files out.

\captree{
  \node[inputnode] (y) {shell yaml\\ \texttt{iea\_s10\_shell.yaml}\\ nodes, elements, sections,\\ elementOrientations, \texttt{reference}};
  \node[funcnode, right=of y] (lr) {\texttt{sg\_mesh.load\_ring\_ref}\\ contour, connectivity, wall\\ normals, \texttt{k22}, laminate \texttt{D\_by};\\ \texttt{reference} $\to$ \texttt{frac}};
  \node[funcnode, below=of lr] (gm) {\texttt{msg\_rm\_plate.rm\_plate\_msg}\\ MSG wall shear block \texttt{G}\\ ($2\times2$), one per layup};
  \node[outnode, right=of gm] (abdg) {\texttt{iea\_s10\_shell\_ABDG.out}\\ $8\times8$ wall law per layup\\ (\texttt{sg\_homo.write\_abdg\_out})};
  \node[funcnode, below=of gm] (ri) {\texttt{sg\_homo.ring\_indep}\\ one-quad-deep prismatic strip,\\ 6 DOF/node, element-constant\\ drilling multipliers, \texttt{mitc4\_g23}};
  \node[datanode, below=of ri] (bun) {bundle: \texttt{Timo} $(6,6)$,\\ \texttt{V0}/\texttt{V1} $(6m,4)$, \texttt{strip},\\ \texttt{layup\_db}, \texttt{frac}};
  \node[outnode, right=of bun] (timo) {\texttt{iea\_s10\_shell\_Timo.out}\\ Timoshenko $6\times6$, timed\\ (\texttt{write\_sc\_K})};
  \node[outnode, below=of bun] (abdy) {\texttt{abd/iea\_s10\_shell\_abd.yaml}\\ (\texttt{sg\_materials.emit\_station\_abd};\\ written only if absent)};
  \node[outnode, right=of abdy] (png) {\texttt{iea\_s10\_mesh.png} (driver),\\ \texttt{iea\_s10\_orient.png}\\ (\texttt{orient\_plot.auto\_emit})};
  \draw[capedge] (y) -- (lr);
  \draw[capedge] (lr) -- (gm);
  \draw[capedge] (gm) -- (abdg);
  \draw[capedge] (gm) -- (ri);
  \draw[capedge] (ri) -- (bun);
  \draw[capedge] (bun) -- (timo);
  \draw[capedge] (bun) -- (abdy);
  \draw[capedge2] (bun) -- (png);
}
...
\label{eq:shell-abdg}
\end{equation}
driven by the eight-component generalized wall strain
\widematrix{\bar{\varepsilon} =
\begin{bmatrix}
...
The $2\times2$ transverse-shear block $\mathbf{G}$ is built by \textbf{one}
route, with no switch to set: the MSG/VAM second-order-energy projection (the
Yu--2002 least-squares construction, Eq.~61 of Yu, Hodges \& Volovoi,
\emph{Computers \& Structures} \textbf{81}:439--454, 2003), computed by
\texttt{opensg\_solid.rm\_plate\_1D.msg\_rm\_plate.rm\_plate\_msg}. The former
alternative \texttt{g\_source="whitney"} --- the coupling-aware
complementary-energy shear flow --- has been retired. The keyword is still
accepted by the old signatures, but passing it explicitly raises a
\texttt{DeprecationWarning} and is \textbf{ignored}, and every emitted ABD cache
records \texttt{g\_source: msg}. The complementary-energy $\mathbf{G}$ survives
only as the internal numerical fallback on a laminate whose MSG $U^*$-fit is not
SPD. Table~\ref{tab:shell-gblocks} shows why this is not a detail on a
foam-cored blade section.
...
The homogenization itself needs no driver. \texttt{opensg} is the unified entry
point: it resolves the engine from the YAML's \texttt{msg:} key (or, when that
key is absent, from the mesh dialect) and hands the file to
\texttt{opensg\_shell}.
...
\noindent
That writes \texttt{iea\_s10\_shell\_Timo.out} and
\texttt{iea\_s10\_shell\_ABDG.out}. Run the driver when you also want the two
figures, which the CLI does not draw:
...
layups --- or after upgrading OpenSG, since a cache left over from an older
version is never refreshed in place --- or the change will not take. And it
records \texttt{g\_source: msg} (with \texttt{reference: mid}, the same
$\mathrm{fraction} = 0.5$ this station's \texttt{reference: center} resolves
to): \texttt{emit\_station\_abd} builds the same MSG wall $\mathbf{G}$ the ring
uses, at the station's own reference surface, so the cached $8\times8$ agrees
with the per-section blocks of \texttt{\_ABDG.out}. It is a cache for downstream
station-level tools such as shell buckling.
...
\begin{Verbatim}[fontsize=\tiny,frame=single,framesep=4pt]
...
The chain below is the whole recovery. Note that it starts by rebuilding the
\emph{same} bundle as Section~\ref{sec:shell-beam}: nothing about the
discretization is re-derived for the recovery.

\captree{
  \node[inputnode] (y) {shell yaml\\ \texttt{iea\_s10\_shell.yaml}};
  \node[funcnode, right=of y] (brb) {\texttt{opensg\_shell.build\_rm\_bundle}\\ the identical ring solve:\\ \texttt{Timo}, \texttt{V0}, \texttt{V1}, \texttt{layup\_db},\\ \texttt{material\_db}, \texttt{frac}};
  \node[inputnode, below=of y] (ff) {beam loads\\ \texttt{ff51\_rmc\_reform.dat}\\ row 10, VABS order,\\ centre-referenced};
  \node[funcnode, below=of brb] (mf) {\texttt{sg\_dehom.\_macro\_fields}\\ $\mathrm{st}=\mathbf{C}_6^{-1}FF$; nodal warping\\ $w=V_0\mathrm{st}_m+V_1\mathrm{st}_{c1}$ and $w'$};
  \node[funcnode, below=of mf] (rs) {\texttt{sg\_dehom.\_rm\_shell\_strain}\\ + \texttt{sg\_assembly.quad\_ops\_indep}\\ per element: 6 shell strains\\ at mid-arc, span gradient \texttt{dE1e}};
  \node[datanode, right=of rs] (grad) {layup-boundary-aware nodal\\ average $\to$ arc gradient \texttt{dE2e}};
  \node[funcnode, below=of rs] (pm) {\texttt{msg\_rm\_plate.rm\_plate\_msg}\\ one 1-D MSG-RM plate SG\\ per layup (6 here)};
  \node[funcnode, below=of pm] (sd) {\texttt{msg\_rm\_plate.msgrm\_strain\_at\_depth}\\ 310 elements $\times$ 9 depths\\ $\to$ 3-D Voigt stress, ply angle};
  \node[outnode, right=of sd] (txt) {\texttt{iea\_s10\_dehom\_shell.txt}\\ 2790 rows, 6 stresses (MPa)\\ \texttt{iea\_s10\_dehom\_stress.png}\\ \texttt{iea\_s10\_cap\_tt.png}};
  \node[outnode, below=of txt] (vtk) {\texttt{write\_dehom\_vtk.py} $\to$\\ \texttt{iea\_s10\_dehom\_shell.vtk}\\ exploded through-thickness quads};
  \draw[capedge] (y) -- (brb);
  \draw[capedge] (brb) -- (mf);
  \draw[capedge] (ff) -- (mf);
  \draw[capedge] (mf) -- (rs);
  \draw[capedge] (rs) -- (grad);
  \draw[capedge] (rs) -- (pm);
  \draw[capedge] (grad) -- (pm);
  \draw[capedge] (pm) -- (sd);
  \draw[capedge] (sd) -- (txt);
  \draw[capedge2] (txt) -- (vtk);
}

\subsection{The input the user edits}

\begin{lstlisting}[style=opensg,caption={The user-input block of beam\_dehom\_shell.py.},basicstyle=\ttfamily\footnotesize]
############### User Input #################################
name = "iea_s10"                  # reads <name>_shell.yaml
...
The recovery is the \texttt{D} argument of the unified entry point: it
homogenizes and then recovers, in one timed run.
...
\noindent
That writes \texttt{iea\_s10\_shell\_dehom.txt}, \texttt{.vtk} and
\texttt{.junc}. Run the driver when you also want the two stress figures, which
the CLI does not draw:

\begin{lstlisting}[style=shellst]
python beam_dehom_shell.py
\end{lstlisting}

\subsection{Junction-aware recovery}
\label{sec:shell-dehom-junction}

A midline shell SG hangs a full through-thickness column off \emph{every}
element mid-arc. Within about a wall thickness of a junction the columns of two
crossing walls therefore cover the \textbf{same material twice}, with two
different wall laws --- which is how a spar-cap ply stress ends up reported at a
web station. \texttt{src/opensg\_shell/sg\_dehom\_junction.py} does not repair
that; it \emph{measures} it, and the YAML header says what to do about it.
Three scalar keys, all optional and all read only on the \texttt{D} route of
\texttt{msg-shell}:
...
key & default & meaning \\
\midrule
\texttt{junction} & \texttt{flag} & the recovery tier: \texttt{off},
  \texttt{flag} or \texttt{exclude} \\
\texttt{junction\_bl} & \texttt{1.0} & $k_{bl}$, the boundary-layer radius in
  units of the junction's \emph{thickest} wall \\
\texttt{junction\_ang} & \texttt{1.0} & the tangent-grouping tolerance of the
  junction detector, in degrees \\
\bottomrule
\end{tabular}
\caption{The three junction keys of the shell dehomogenization header.}
\label{tab:shell-junction-keys}
\end{table}
...
tier & behaviour \\
\midrule
\texttt{off} & the pre-junction behaviour exactly: no extra columns, no
  sidecar, no census, bit-identical files \\
\texttt{flag} & \textbf{the default}, and \textbf{non-mutating}: two extra
  columns \texttt{jflag} and \texttt{jdist} at the \emph{end} of
  \texttt{<base>\_dehom.txt}, the same two as extra \texttt{CELL\_DATA} scalars
  in \texttt{<base>\_dehom.vtk}, the \texttt{<base>\_dehom.junc} sidecar, and a
  printed census. It changes no field value \\
\texttt{exclude} & as \texttt{flag}, plus \texttt{NaN} in the 15 field columns
  of every duplicate (\texttt{jflag} 3) row, so the point cloud
  \emph{partitions} the material instead of double-covering it. The row itself
  stays, so row $i$ is still element $\lfloor i/n_{\mathrm{depth}}\rfloor$ at
  depth $i \bmod n_{\mathrm{depth}}$. The \texttt{.vtk} values are never
  NaN-ed --- threshold on \texttt{jflag} instead \\
\bottomrule
\end{tabular}
\caption{The three implemented junction tiers. Because the two columns are
appended at the end, every existing column index is untouched.}
\label{tab:shell-junction-tiers}
\end{table}

A fourth tier, \texttt{patch}, is \textbf{designed but deliberately not
implemented} and raises if requested. The corner micro-solve it would call is
admissible only for two straight walls of uniform layup crossing at one node,
which most real blade junctions --- a web landing on a tapering spar cap, a
three-way leading-edge node, coincident doublers --- are not.

The two new columns are defined as follows. \texttt{jflag} is \texttt{0} clean,
\texttt{1} near, \texttt{2} overlapped and this wall owns the block, \texttt{3}
duplicate (another wall owns it), and \texttt{4} is reserved for \texttt{patch}
and is never emitted. \texttt{jdist} is $|p - J| / \max_i t_i$ at the nearest
junction node $J$ (infinite when the section has no junction at all), so
\texttt{jflag} $\ge 1$ holds exactly when \texttt{jdist} $\le k_{bl}$.

Both remaining keys are modelling choices, not derived lengths.
\texttt{junction\_bl} sets $k_{bl}$: the RM wall solution is polluted within
roughly one wall thickness of a crossing, and $1.0$ is the conventional
Saint-Venant decay scale, nothing more; widen it to be conservative.
\texttt{junction\_ang} is the detector's tangent-grouping tolerance, and its
default of $1.0^\circ$ is the homogenization census value and deliberately
tight: on a curved airfoil contour it also calls every ${>}1^\circ$ contour
\emph{kink} a junction. Raise it to $5$--$15^\circ$ for a census that lists the
wall crossings only.

\begin{Verbatim}[frame=single,framesep=4pt,fontsize=\small]
 junctions : 97 detected (5 T, 91 L, 1 unresolved)
 stations  : 2790 total, 1032 near, 56 overlapped (50 duplicates)
\end{Verbatim}

That is the measured census of this station, read from
\texttt{iea\_s10\_shell\_dehom.junc}: of 97 detected junctions only a handful
are genuine wall crossings, the rest being discretized curvature at the default
$1.0^\circ$ tolerance, and 1032 of the 2790 recovery stations fall inside some
boundary layer while only 56 are truly overlapped.

One warning. This is \textbf{not} the same switch as the \emph{homogenization}
junction correction \texttt{build\_solid\_bundle(junction="census"} $|$
\texttt{"micro"} $|$ \texttt{"microcell")} of
Section~\ref{sec:shell-solid-xsec}. That one is an energy correction with its
own validity domain and it stays off by default; the tier here is recovery
bookkeeping, and at \texttt{flag} it corrects nothing at all.
...
\begin{lstlisting}[style=opensg,caption={The recovery chain in miniature: one element, one depth.},basicstyle=\ttfamily\footnotesize]
import numpy as np
...
\begin{Verbatim}[frame=single,framesep=4pt,fontsize=\scriptsize]
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.86\linewidth]{\FIGDIR/pv_shell_dehom_s11.png}\\[2mm]
\includegraphics[width=0.86\linewidth]{\FIGDIR/pv_shell_dehom_s12.png}
\caption{Recovered $\sigma_{11}$ (top) and $\sigma_{12}$ (bottom) over the
cross-section, in MPa, rendered from
\texttt{iea\_s10\_dehom\_shell.vtk}. These are \emph{conformal exploded-mesh}
contours: \texttt{write\_dehom\_vtk.py} emits one filled quad per contour
element per through-thickness interval $\zeta \in [k/9,\,(k+1)/9]$, so the 310
elements become $310\times 9 = 2790$ patches carrying 11\,160 points of their
own. Each patch is flat-shaded with the stress of its own recovery station as
\texttt{CELL\_DATA}, so no value is averaged across an element boundary or onto
a shared node, and because the nine intervals tile the full layup thickness $h$
the wall is drawn at its true thickness rather than as a line.}
...
The VTK file is not written by \texttt{beam\_dehom\_shell.py} itself. The
sibling script \texttt{write\_dehom\_vtk.py} imports that driver, so the physics
is not duplicated, and re-packages its recovered values as the exploded quad
grid; \texttt{docs/latex/render\_pv.py} then renders the two panels above with
\texttt{pvpython}.
...
\bottomrule
\end{tabular}
\caption{Extremes of \texttt{iea\_s10\_dehom\_shell.txt} under the load state
$FF$ of row 10 of \texttt{ff51\_rmc\_reform.dat}.}
...
Every case in this section is a two-command run, and the two commands are the
two input nodes of the chain below: a generator that writes the 1-D shell YAML,
then a driver that calls \texttt{build\_solid\_bundle} on it.

\captree{
  \node[inputnode] (gen) {generator, e.g.\\ \texttt{make\_deo\_cellular\_yamls.py}\\ wall lengths $l$, $h$, angle $\theta$,\\ thickness $t$, \texttt{nseg}};
  \node[inputnode, below=of gen] (y) {1-D shell yaml, e.g.\\ \texttt{deo\_cellular\_+15\_1Dshell.yaml}\\ \texttt{reference: center}};
  \node[funcnode, right=of y] (bsb) {\texttt{opensg\_shell.build\_solid\_bundle}\\ \texttt{cell\_area} $=\omega$: the \texttt{omega:}\\ header key, else the MEASURED\\ node bounding box; \texttt{periodic=True}};
  \node[funcnode, below=of bsb] (lr) {\texttt{sg\_mesh.load\_ring\_ref} +\\ \texttt{msg\_rm\_plate.rm\_plate\_msg}\\ wall $8\times8$ with the MSG \texttt{G}};
  \node[outnode, right=of lr] (abdg) {\texttt{deo\_cellular\_+15\_1Dshell\_ABDG.out}\\ (\texttt{sg\_homo.write\_abdg\_out})};
  \node[funcnode, below=of lr] (rs) {\texttt{sg\_homo.ring\_solid}\\ \texttt{sg\_periodicity.mesh\_to\_periodic\_sparse\_assembly\_map}\\ ties faces, edges, corners;\\ \texttt{assemble\_solid\_strip} +\\ \texttt{assemble\_solid\_macro}};
  \node[datanode, below=of rs] (deff) {\texttt{D\_eff} $(6,6)$ per unit length,\\ \texttt{C3D} $=$ \texttt{D\_eff}/\texttt{cell\_area}\\ on \texttt{GBAR\_ORDER}};
  \node[outnode, right=of deff] (c3d) {\texttt{deo\_cellular\_+15\_1Dshell\_C3D.out}\\ (\texttt{write\_sc\_K}, timed)};
  \node[outnode, below=of deff] (dat) {\texttt{deo\_cellular\_+15\_msg\_shell.out},\\ \texttt{deo\_cellular\_compare.dat},\\ \texttt{deo\_cellular\_+15\_mesh.png}};
  \node[outnode, right=of dat] (ec) {\texttt{square\_solid\_msg\_shell.out}\\ $6\times6$ plus 9 engineering constants\\ (\texttt{sg\_homo.elastic\_constants})};
  \draw[capedge] (gen) -- (y);
  \draw[capedge] (y) -- (bsb);
  \draw[capedge] (bsb) -- (lr);
  \draw[capedge] (lr) -- (abdg);
  \draw[capedge] (lr) -- (rs);
  \draw[capedge] (rs) -- (deff);
  \draw[capedge] (deff) -- (c3d);
  \draw[capedge] (deff) -- (dat);
  \draw[capedge2] (deff) -- (ec);
}

The strain order is the solid Voigt order
\widematrix{\bar{\Gamma} = \begin{bmatrix}
...
\begin{lstlisting}[style=opensg,caption={The one call for equivalent-solid properties from a cross-section SG.},basicstyle=\ttfamily\footnotesize]
...
\texttt{area\_source} reads \texttt{"user"}); if you omit it in a direct library
call, the convex-hull area of the contour is used as a legacy fallback and
\texttt{area\_source} reads \texttt{"hull"}. The \texttt{opensg <yaml>} CLI
never takes that fallback: it supplies the measure itself
(\texttt{cli.sg\_cell\_area}) as the \texttt{omega:} header key when the file
declares one, and otherwise as the \textbf{measured periodic cell} --- the node
\emph{bounding box}, because the assembly map ties opposite box faces. The
bounding box is not the convex hull of the contour; the two differ on any
non-convex cell, which is every honeycomb and every lattice in this section.
\texttt{build\_solid\_bundle} writes
...
\begin{lstlisting}[style=shellst,basicstyle=\ttfamily\footnotesize]
...
Every driver in this folder redraws the 1-D SG it just solved, straight from the
\texttt{nodes} and \texttt{elements} blocks of its own YAML, so
Figures~\ref{fig:deo-cellular-mesh}, \ref{fig:deo-square-mesh}
and~\ref{fig:deo-square-tube-mesh} are the meshes the numbers came from and not
sketches.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.47\linewidth]{\FIGDIR/deo_cellular_p15_mesh.png}\hfill
\includegraphics[width=0.47\linewidth]{\FIGDIR/deo_cellular_m15_mesh.png}
\caption{The two cellular 1-D shell SGs, written as
\texttt{deo\_cellular\_+15\_mesh.png} and \texttt{deo\_cellular\_-15\_mesh.png}
by \texttt{deo\_cellular\_solid\_props.py}. Left: the hexagonal cell at
$\theta = +15^\circ$, whose inclined walls splay outward. Right: the re-entrant
(auxetic) cell at $\theta = -15^\circ$, whose inclined walls fold inward, which
is what reverses the sign of $C_{13}$ and $C_{23}$. Both are the periodic double
cell of nine walls: one full vertical column of length $h = 10$ cm, two
box-clipped column halves, two full inclined walls of length $l = 10$ cm and
four clipped inclined stubs, all $t = 5$ mm aluminium at \texttt{nseg = 10}
elements per wall of length $l$. Node dots mark the two-node line elements.}
\label{fig:deo-cellular-mesh}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.47\linewidth]{\FIGDIR/deo_hier_square_mesh.png}\hfill
\includegraphics[width=0.47\linewidth]{\FIGDIR/deo_composite_square_mesh.png}
\caption{The two square-lattice 1-D shell SGs. Left: the hierarchical square
(\texttt{deo\_square\_mesh.png}, from \texttt{deo\_square\_solid\_props.py}) --
a closed inner square of half-side $r = 5$ cm built from eight half-walls, plus
four ligaments running out to the cell boundary at $\pm R = \pm 10$ cm, twelve
walls in all at $t = 5$ mm and \texttt{nseg = 10} elements per wall. Right: the
composite square (\texttt{deo\_composite\_square\_mesh.png}, from
\texttt{deo\_composite\_square\_props.py}) -- a $25.4\times25.4$ mm cell with
two horizontal walls at $y_3 = \pm 5.84$ mm and one vertical wall spanning the
full cell height through the centre, generated as seven segments of
\texttt{nseg = 16} elements each, every wall a $[15]_8$ laminate of
$t = 1.016$ mm. Both are axis-aligned $0/90$ lattices, which is why both lose
the membrane part of $C_{23}$.}
\label{fig:deo-square-mesh}
\end{figure}
...
$E_2 = E_3 = 9.79$, $G = 6.136$ GPa, $\nu = 0.42$. The right panel of
Figure~\ref{fig:deo-square-mesh} is that mesh.
...
place to start. It reads \texttt{square\_tube\_1Dshell.yaml}, whose geometry is
\texttt{a, t\_w = 1.0, 0.03}. This is the one case in the section that does
\emph{not} want the measured bounding box: the YAML therefore declares
\texttt{omega: 0.12} in its header, which is $4at_w$, the \emph{wall material
area} and the same measure the solid engines use, so the two routes are directly
comparable. Because the measure rides in the header, the run is the plain
one-liner

\begin{lstlisting}[style=shellst]
opensg square_tube_1Dshell.yaml
\end{lstlisting}

\noindent
which writes \texttt{square\_tube\_1Dshell\_C3D.out}, carrying the $6\times6$
and the nine engineering constants under the banner

\begin{Verbatim}[fontsize=\small,frame=single,framesep=4pt]
 OpenSG msg-shell equivalent 3D solid (cross-section SG), omega 0.12 (user)
\end{Verbatim}

\noindent
where \texttt{(user)} is \texttt{area\_source} recording that the measure was
supplied rather than defaulted to the hull. The driver
\texttt{solid\_props\_msg\_shell.py} adds the mesh figure and a second copy of
the same law, \texttt{square\_solid\_msg\_shell.out}, laid out for the
comparison scripts.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.47\linewidth]{\FIGDIR/deo_square_tube_mesh.png}
\caption{The isotropic square-tube 1-D shell SG, written by
\texttt{solid\_props\_msg\_shell.py} as \texttt{square\_tube\_mesh.png}. A
single closed cell: four straight walls on the midline of a square of side
$a = 1$ m, wall thickness $t_w = 0.03$ m carried as material data, centre
reference. This is the one case whose measure is the wall material area
$4at_w$ rather than a bounding-box area, which is what makes it directly
comparable with a 2-D solid run of the same tube.}
\label{fig:deo-square-tube-mesh}
\end{figure}
...
$6\times6$, on the same solid Voigt order $\bar{\Gamma}$ of
Section~\ref{sec:shell-solid-xsec}. \texttt{shell\_sg3d} solves it when the
...
The example is
\texttt{examples/OpenSG\_shell/4\_get\_solid\_props\_from\_shell\_3D\_SG}, and
its data flow is the chain below. The only branch point is the \texttt{boundary}
argument.

\captree{
  \node[inputnode] (msh) {gmsh 2.2 triangle mesh\\ \texttt{schwarz\_p\_D2\_shell.msh}\\ plus \texttt{T}, \texttt{E}, \texttt{nu}};
  \node[funcnode, right=of msh] (mk) {\texttt{make\_schwarz\_yaml.py}\\ per-triangle frame $e_3$ = unit normal,\\ $e_2$ along edge 1, $e_1=e_2\times e_3$;\\ one ply \texttt{[alu, T, 0.0]}};
  \node[inputnode, below=of mk] (y) {3-D shell yaml\\ \texttt{schwarz\_p\_3Dshell.yaml}\\ 13\,536 nodes, 26\,360 tris};
  \node[funcnode, below=of y] (s3d) {\texttt{sg\_homo.shell\_sg3d}\\ \texttt{boundary="periodic"} (default)\\ or \texttt{"aperiodic"}; \texttt{omega=}};
  \node[funcnode, below=of s3d] (mat) {\texttt{sg\_materials.\_material\_by\_section}\\ + \texttt{msg\_rm\_plate.rm\_plate\_msg}\\ wall $6\times6$ \texttt{De} and MSG \texttt{Gm};\\ junction-line count};
  \node[funcnode, below=of mat] (ops) {\texttt{sg\_homo.solid\_fluct\_ops\_batch}\\ + \texttt{sg\_homo.solid\_macro\_ops\_batch}\\ 4 Gauss points $\to$ \texttt{K}, \texttt{Dhe}, \texttt{Dee}};
  \node[datanode, right=of ops] (tie) {\texttt{sg\_periodicity.mesh\_to\_periodic\_sparse\_assembly\_map}\\ (periodic) \emph{or} bounding-box\\ $w=0$ Dirichlet (aperiodic)};
  \node[datanode, below=of ops] (v0) {\texttt{splu} solve $\to$ \texttt{V0};\\ \texttt{D\_eff} $=$ \texttt{Dee} $+$ \texttt{V0}$^T$\texttt{Dhe};\\ \texttt{C3D} $=$ \texttt{D\_eff}$/\omega$};
  \node[outnode, right=of v0] (out) {\texttt{schwarz\_p\_3Dshell\_C3D.out}\\ per unit-cell volume, timed\\ (\texttt{write\_sc\_K})};
  \node[outnode, below=of v0] (png) {\texttt{schwarz\_p\_mesh.png}\\ (\texttt{schwarz\_solid\_props.py})};
  \node[outnode, right=of png] (cmp) {\texttt{compare\_boundary\_modes.py} $\to$\\ \texttt{schwarz\_t0.036457\_periodic.out}\\ (one per mode), \texttt{boundary\_compare.dat}};
  \draw[capedge] (msh) -- (mk);
  \draw[capedge] (mk) -- (y);
  \draw[capedge] (y) -- (s3d);
  \draw[capedge] (s3d) -- (mat);
  \draw[capedge] (mat) -- (ops);
  \draw[capedge] (tie) -- (ops);
  \draw[capedge] (ops) -- (v0);
  \draw[capedge] (v0) -- (out);
  \draw[capedge] (v0) -- (png);
  \draw[capedge2] (out) -- (cmp);
}
...
The generator writes the YAML; the homogenization is then the unified one-liner,
because everything the route needs --- the layup, the material and the SG
measure --- is in that file. The third command is the two-mode boundary
comparison of Table~\ref{tab:schwarz-boundary}, which no single run can produce.
...
\noindent
\texttt{schwarz\_solid\_props.py} runs the same homogenization from Python and
adds the mesh figure.

\begin{lstlisting}[style=opensg,caption={The two boundary treatments of shell\_sg3d.},basicstyle=\ttfamily\scriptsize]
...
For a 3-D SG the measure $\omega$ is a \textbf{volume}, not an area: it defaults
to the node \textbf{bounding-box volume}
$\mathrm{ptp}(y_1)\,\mathrm{ptp}(y_2)\,\mathrm{ptp}(y_3)$, measured from the
mesh --- the volume the equivalent continuum occupies, and the same cell the
periodic assembly map ties. It is the same convention \texttt{opensg\_solid}
uses, so the returned \texttt{C3D}, the line the CLI prints and
\texttt{<base>\_C3D.out} are one and the same matrix, directly comparable with a
solid SwiftComp \texttt{.K}. An explicit \texttt{omega=} argument (or the YAML's
\texttt{omega:} header key, which the CLI forwards here) overrides it --- for
example with the midsurface \emph{surface} area, which the result dict also
returns as \texttt{"surface\_area"}, for a per-unit-area wall law.

\subsection{What comes out}

\texttt{shell\_sg3d} writes two files, \texttt{schwarz\_p\_3Dshell\_C3D.out} and
\texttt{schwarz\_p\_3Dshell\_ABDG.out}, and the driver writes
\texttt{schwarz\_p\_mesh.png}.

The \texttt{\_ABDG.out} is the step-1 record: every route that reduces a layup
to a wall plate law now emits it, the cross-section rings of
Sections~\ref{sec:shell-beam} and~\ref{sec:shell-solid-xsec} and the 3-D shell
SG alike. It is an $8\times8$ Reissner--Mindlin ABDG stiffness plus its
compliance, one block per section, opening with
...
\noindent
The Schwarz-P surface has a single section, headed
\texttt{section 0: layup\_0  layup [['alu', 0.036457, 0.0]]}, and its wall law
is $A_{11} = 2.7643220\times10^{9}$, $A_{12} = 8.2929659\times10^{8}$,
$A_{66} = 9.6751264\times10^{8}$ N/m, $D_{11} = 3.0617465\times10^{5}$
N$\cdot$m and $G_{11} = G_{22} = 8.5193106\times10^{8}$ N/m --- the plate law of
a single $36.457$ mm aluminium ply, which is the cheapest possible check that
the thickness and the material reached the solver intact.

The banner line of the \texttt{\_C3D.out} records the whole configuration, and
the default periodic run at $t = 0.036457$ gives:
...
\captree{
  \node[inputnode] (y) {3-D shell segment yaml\\ \texttt{meshes/seg\_iso\_hR0.1.yaml} or\\ \texttt{BAR\_URC\_numEl\_52\_segment\_12.yaml}\\ quads, sections, materials};
  \node[funcnode, below=of y] (ex) {\texttt{sg\_mesh.extract}\\ free edges (one owner quad),\\ connected components, axis from the\\ two most separated centroids};
  \node[outnode, right=of ex] (byaml) {\texttt{seg\_iso\_hR0.1\_boundaries.npz}\\ carrying \texttt{L\_node2seg}, \texttt{R\_node2seg};\\ \texttt{boundary\_seg\_iso\_hR0.1\_boundaries\_L.yaml} and \texttt{\_R.yaml};\\ three \texttt{orient\_*.png}};
  \node[funcnode, below=of ex] (ri) {\texttt{sg\_homo.ring\_indep}, once per side\\ $\to$ \texttt{C6}, \texttt{V0}, \texttt{V1} (6 DOF/node,\\ drilling $\omega_3$ included)};
  \node[funcnode, below=of ri] (asm) {\texttt{sg\_assembly.assemble\_segment\_indep}\\ \texttt{Dhh}, \texttt{Dhe}, \texttt{Dee}, \texttt{Dhl}, \texttt{Dll}, \texttt{Dle}\\ (\texttt{shear="full"}, \texttt{pen=0})};
  \node[funcnode, right=of asm] (con) {\texttt{sg\_assembly.assemble\_constraint}\\ $\to$ \texttt{Gc}, \texttt{Gl}, \texttt{Ge};\\ \texttt{sg\_homo.\_augment\_drilling};\\ \texttt{build\_C\_Psi\_segment6}};
  \node[funcnode, below=of asm] (sol) {\texttt{sg\_assembly.dirichlet\_factor} once,\\ then \texttt{dirichlet\_solve\_fac} twice:\\ \texttt{V0} then \texttt{V1}, shared factorization,\\ ring fields as the end Dirichlet data};
  \node[datanode, below=of sol] (s6) {\texttt{msg\_solver.prepare\_v1\_rhs} +\\ \texttt{finalize\_v1\_and\_compute\_deff}\\ $\to$ \texttt{S6} $(6,6)$, each block $/L$};
  \node[outnode, right=of s6] (out) {\texttt{seg\_iso\_hR0.1\_Timo.out}\\ SwiftComp layout, timed\\ (\texttt{write\_sc\_K})};
  \node[outnode, below=of s6] (dat) {\texttt{seg\_ring\_identity.dat} (driver)\\ \texttt{bar\_urc\_segment12.dat}};
  \draw[capedge] (y) -- (ex);
  \draw[capedge] (ex) -- (byaml);
  \draw[capedge] (ex) -- (ri);
  \draw[capedge] (ri) -- (asm);
  \draw[capedge] (asm) -- (con);
  \draw[capedge] (asm) -- (sol);
  \draw[capedge] (con) -- (sol);
  \draw[capedge] (ri.west) -- ++(-5mm,0) |- (sol.west);
  \draw[capedge] (sol) -- (s6);
  \draw[capedge] (s6) -- (out);
  \draw[capedge] (s6) -- (dat);
}
...
\begin{lstlisting}[style=opensg,caption={The parameter block of make\_cylinder\_segment.py.},basicstyle=\ttfamily\footnotesize]
