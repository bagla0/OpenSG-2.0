...
Figure~\ref{fig:windio:pipeline} is the entire round trip drawn as one data
flow: which file you edit, which function consumes it, what that function hands
on, and which file it leaves behind. Read it once before running anything --
the two dashed edges in the middle are the only place where the chain leaves
OpenSG, and everything above and below them is reproducible from the shipped
example directory. The rest of this chapter walks the same graph stage by stage,
with a smaller tree at the head of each stage that opens up the box.

\begin{figure}[htbp]
\captree{
  \node[inputnode] (w) {windIO blade description\\ \texttt{IEA-22-280-RWT.yaml}\\ v1 or v2, auto-detected};
  \node[funcnode, below=of w] (io) {\texttt{opensg\_io.load\_blade}\\ \texttt{opensg\_io.build\_cross\_section}\\ \texttt{opensg\_io.emit\_opensg\_yaml}};
  \node[outnode, below=of io] (ys) {\texttt{iea\_s00\_shell.yaml} through\\ \texttt{iea\_s50\_shell.yaml}\\ one ring SG per span station};
  \node[funcnode, below=of ys] (rm) {\texttt{sg\_homo.build\_rm\_bundle}\\ \texttt{sg\_mesh.load\_ring\_ref},\\ \texttt{rm\_plate\_msg} wall $\mathbf{G}$,\\ \texttt{sg\_homo.ring\_indep}: $\mathbf{V}_0$, $\mathbf{V}_1$};
  \node[outnode, below=of rm] (to) {\texttt{iea\_s10\_shell\_Timo.out}\\ \texttt{iea\_s10\_shell\_ABDG.out}\\ Timoshenko $6\times6$ per station};
  \node[outnode, below=of to] (tab) {\texttt{iea51\_beam\_props.dat}\\ the spanwise table, 49 stations};
  \node[funcnode, below=of tab] (bd) {\texttt{opensg\_io.openfast\_io}:\\ \texttt{timo\_to\_beamdyn}\\ \texttt{write\_beamdyn\_blade}};

  \node[outnode, right=20mm of w] (bdf) {\texttt{blade\_BeamDyn\_Blade.dat}\\ BeamDyn blade input;\\ the mass block is yours};
  \node[datanode, below=of bdf] (ext) {\textbf{external}: the OpenFAST\\ BeamDyn run, outside OpenSG};
  \node[inputnode, below=of ext] (ff) {\texttt{ff51\_rmc\_reform.dat}\\ per-station sectional loads\\ $[F_1\,F_2\,F_3\,M_1\,M_2\,M_3]$};
  \node[funcnode, below=of ff] (da) {step 5a, the section:\\ \texttt{sg\_dehom.\_macro\_fields}\\ \texttt{sg\_dehom.\_rm\_shell\_strain}};
  \node[outnode, below=of da] (pf) {\texttt{iea\_s10\_elem\_plate\_forces.dat}\\ $\mathbf{F}_6 = \mathbf{A}_6\,\mathbf{s}_6$ per element};
  \node[funcnode, below=of pf] (db) {step 5b, the wall:\\ \texttt{msg\_rm\_plate.rm\_plate\_msg}\\ \texttt{msgrm\_strain\_at\_depth\_batch}};
  \node[outnode, below=of db] (sig) {\texttt{iea\_s10\_dehom\_v2.txt}\\ \texttt{iea\_s10\_dehom\_v2.npz}\\ recovered 3-D wall stress};

  \draw[capedge] (w) -- (io);   \draw[capedge] (io) -- (ys);
  \draw[capedge] (ys) -- (rm);  \draw[capedge] (rm) -- (to);
  \draw[capedge] (to) -- (tab); \draw[capedge] (tab) -- (bd);
  \draw[capedge] (bd.east) -- ++(0.55,0) |- (bdf.west);
  \draw[capedge2] (bdf) -- (ext);  \draw[capedge2] (ext) -- (ff);
  \draw[capedge] (ff) -- (da);  \draw[capedge] (da) -- (pf);
  \draw[capedge] (pf) -- (db);  \draw[capedge] (db) -- (sig);
}
\caption{The whole blade round trip as one data flow. The left column is
         stages 1 to 3, the right column stages 3 to 5, and the two dashed
         edges are the OpenFAST BeamDyn run, which OpenSG neither contains nor
         launches. Yellow nodes are files you edit, blue nodes are functions
         that run, grey nodes are quantities that pass between them, and green
         nodes are files written to disk.}
\label{fig:windio:pipeline}
\end{figure}
...
The four functions and the fork between the two output forms:

\captree{
  \node[inputnode] (b) {windIO blade\\ \texttt{IEA-22-280-RWT.yaml}};
  \node[funcnode, below=of b] (lb) {\texttt{opensg\_io.load\_blade}\\ windIO v1 or v2, auto-detected};
  \node[funcnode, below=of lb] (bx) {\texttt{opensg\_io.build\_cross\_section}\\ outer shape, layup stacks and the\\ material database interpolated to $r/R$};
  \node[datanode, below=of bx] (cs) {\texttt{cs}: one cross-section};
  \node[funcnode, below=of cs] (ey) {\texttt{opensg\_io.emit\_opensg\_yaml}};
  \node[outnode, below=of ey] (y1) {\texttt{iea\_s10\_shell.yaml}\\ the 1-D shell SG read\\ by stages 2 and 5};
  \node[funcnode, right=6mm of cs] (ep) {\texttt{opensg\_io.emit\_prevabs}\\ then PreVABS meshes it};
  \node[outnode, below=of ep] (y2) {2-D solid cross-section\\ $\rightarrow$ \texttt{iea\_s10.sg.SM}, the VABS\\ reference that validates stage 5};

  \draw[capedge] (b) -- (lb);  \draw[capedge] (lb) -- (bx);
  \draw[capedge] (bx) -- (cs); \draw[capedge] (cs) -- (ey);
  \draw[capedge] (ey) -- (y1);
  \draw[capedge2] (cs) -- (ep); \draw[capedge2] (ep) -- (y2);
}
...
\begin{lstlisting}[style=opensg,caption={Building one station's 1-D shell SG and its PreVABS twin.},basicstyle=\ttfamily\footnotesize]
...
One call does all of it, but it is worth knowing what that call chains together,
because every intermediate in the following tree is an entry of the returned
bundle that stage 5 reads back:

\captree{
  \node[inputnode] (y) {\texttt{iea\_s10\_shell.yaml}\\ its own \texttt{reference: center}};
  \node[funcnode, below=of y] (lr) {\texttt{sg\_mesh.load\_ring\_ref}\\ contour \texttt{rx}, \texttt{cells}, \texttt{rsub}, \texttt{re3}, \texttt{ax}, \texttt{cross};\\ it calls \texttt{sg\_materials.\_material\_by\_section}\\ for the wall ABD $6\times6$ and $\mathbf{G}$ per section,\\ and \texttt{sg\_assembly.compute\_k22} for \texttt{k22}};
  \node[funcnode, below=of lr] (gm) {\texttt{msg\_rm\_plate.rm\_plate\_msg}\\ the MSG (Yu 2002 least-squares)\\ wall $\mathbf{G}$ -- the only route,\\ no switch};
  \node[funcnode, below=of gm] (ri) {\texttt{sg\_homo.ring\_indep}\\ one KKT factorization, two solves:\\ $\mathbf{V}_0$ then $\mathbf{V}_1$, then\\ \texttt{finalize\_v1\_and\_compute\_deff}};
  \node[outnode, below=of ri] (o1) {\texttt{iea\_s10\_shell\_Timo.out} (\texttt{write\_sc\_K})\\ \texttt{iea\_s10\_shell\_ABDG.out} (\texttt{write\_abdg\_out})\\ \texttt{abd/iea\_s10\_shell\_abd.yaml}\\ (\texttt{sg\_materials.emit\_station\_abd})};
  \node[datanode, right=6mm of ri] (bu) {the bundle \texttt{B}:\\ \texttt{Timo}, \texttt{V0}, \texttt{V1}, \texttt{strip},\\ \texttt{layup\_db}, \texttt{material\_db},\\ \texttt{frac} $\rightarrow$ stage 5};

  \draw[capedge] (y) -- (lr);  \draw[capedge] (lr) -- (gm);
  \draw[capedge] (gm) -- (ri);
  \draw[capedge] (ri) -- (o1); \draw[capedge] (ri) -- (bu);
}
...
The script is kept here because it does more than homogenize: it also writes
the bare $6\times6$ \texttt{iea\_s10\_beam\_Timo.out} and the ring mesh figure.
The homogenization on its own is the unified command,

\begin{lstlisting}[style=shellst]
opensg iea_s10_shell.yaml
\end{lstlisting}

which dispatches on the yaml's \texttt{msg: shell} key --- or, absent that key,
on the mesh dialect --- and hands the argument to \texttt{opensg\_shell}
unchanged. The macro model comes from the yaml header, and \emph{that} header
is what decides which beam law you get: \texttt{refined: 1} asks for the
Timoshenko $6\times6$ and \texttt{iea\_s10\_shell\_Timo.out}, while the default
\texttt{refined: 0} reports the classical Euler-Bernoulli $4\times4$ into
\texttt{iea\_s10\_shell\_EB.out}.
...
\texttt{build\_rm\_bundle(shell\_yaml, ref=None, shear="mitc4\_g23")}. Passing
\texttt{ref=None} means the YAML's own \texttt{reference} field decides, which
is why homogenization and dehomogenization cannot silently disagree; pass an
explicit \texttt{ref} only to override.

There is no wall-$\mathbf{G}$ selector. The $2\times2$ transverse-shear block
of every wall plate law is built by one route: the MSG/VAM
second-order-energy (Yu 2002 least-squares) projection computed by
\texttt{opensg\_solid.rm\_plate\_1D.msg\_rm\_plate.rm\_plate\_msg}. The
retired \texttt{g\_source} argument is still \emph{accepted} by
\texttt{build\_rm\_bundle}, \texttt{build\_solid\_bundle},
\texttt{shell\_sg3d} and \texttt{emit\_station\_abd} so that old call sites do
not break, but an explicit value raises a \texttt{DeprecationWarning} and is
ignored. The complementary-energy (Whitney 1973) shear-flow $\mathbf{G}$
survives only as the internal numerical fallback on a laminate whose MSG
$U^*$ fit is not positive definite; it is not a user-selectable alternative.
...
\texttt{material\_db}, and \texttt{frac}, \texttt{ref} and \texttt{g\_source}
--- the last a record, always \texttt{"msg"}, not a choice.
...
same surface as the ring's \texttt{center}. And it records
\texttt{g\_source: msg}, because there is only one wall-$\mathbf{G}$ route:
the two transverse-shear rows of the cached $8\times8$ are the same MSG
$\mathbf{G}$ the ring homogenization itself used, so the cache and the ring
agree block for block and a recovery driven from the cache cannot silently
disagree with the $6\times6$. For \texttt{iea\_s10} the file reports
...
Drawn as a data flow, with the two dashed edges marking the part OpenSG does
not perform:

\captree{
  \node[datanode] (ks) {\texttt{K\_timo\_list}: the per-station\\ \texttt{B["Timo"]} $6\times6$, VABS order};
  \node[funcnode, below=of ks] (t2b) {\texttt{openfast\_io.timo\_to\_beamdyn}\\ symmetric permutation of rows and\\ columns, \texttt{q = [1, 2, 0, 4, 5, 3]}};
  \node[funcnode, below=of t2b] (wbb) {\texttt{openfast\_io.write\_beamdyn\_blade}\\ \texttt{station\_total}, \texttt{damp\_type} $=1$,\\ a stiffness block then a mass block\\ for every station};
  \node[inputnode, right=6mm of wbb] (ml) {\texttt{M\_list}: your own\\ sectional mass matrices;\\ omitted means a $6\times6$\\ of zeros per station};
  \node[outnode, below=of wbb] (bdf) {\texttt{blade\_BeamDyn\_Blade.dat}};
  \node[datanode, below=of bdf] (run) {\textbf{external}: the OpenFAST\\ BeamDyn run, installed\\ and driven by you};
  \node[inputnode, below=of run] (ffo) {the per-station sectional\\ loads that come back as\\ the stage 4 table};

  \draw[capedge] (ks) -- (t2b);  \draw[capedge] (t2b) -- (wbb);
  \draw[capedge] (ml) -- (wbb);  \draw[capedge] (wbb) -- (bdf);
  \draw[capedge2] (bdf) -- (run); \draw[capedge2] (run) -- (ffo);
}
...
\begin{lstlisting}[style=opensg,caption={ff51\_rmc\_reform.yaml: the FF table as a self-describing file.},basicstyle=\ttfamily\footnotesize]
...
\begin{lstlisting}[style=shellst,basicstyle=\ttfamily\scriptsize]
...
The whole of stage 4 is therefore a short chain, and the only thing it decides
is which row of the table stage 5 will use:

\captree{
  \node[inputnode] (d) {\texttt{ff51\_rmc\_reform.dat}\\ 51 rows:\\ \texttt{eta F1 F2 F3 M1 M2 M3}};
  \node[funcnode, below=of d] (cv) {\texttt{ff\_to\_yaml.convert}\\ raises if fewer than seven columns};
  \node[outnode, below=of cv] (yy) {\texttt{ff51\_rmc\_reform.yaml}\\ order, frame and units recorded\\ next to the data};
  \node[funcnode, below=of yy] (sf) {\texttt{ff\_to\_yaml.station\_ff}\\ selected by nearest \texttt{eta} or by \texttt{row}};
  \node[datanode, below=of sf] (fv) {\texttt{FF} $(6,)$ and $\eta$ for one\\ station $\rightarrow$ stage 5};

  \draw[capedge] (d) -- (cv);  \draw[capedge] (cv) -- (yy);
  \draw[capedge] (yy) -- (sf); \draw[capedge] (sf) -- (fv);
}
...
The two steps and the interface file between them:

\captree{
  \node[inputnode] (bb) {the bundle \texttt{B} of stage 2\\ and \texttt{FF} of stage 4};
  \node[funcnode, below=of bb] (mf) {\texttt{sg\_dehom.\_macro\_fields}\\ $\bar{\boldsymbol{\varepsilon}} = \mathbf{C}_6^{-1}\mathbf{FF}$, then\\ $\mathbf{w} = \mathbf{V}_0\bar{\boldsymbol{\varepsilon}} + \mathbf{V}_1\boldsymbol{\varepsilon}'$};
  \node[funcnode, below=of mf] (rs) {\texttt{sg\_dehom.\_rm\_shell\_strain}\\ $\mathbf{s}_6$ per element; the gradients from\\ \texttt{sg\_assembly.quad\_ops\_indep} and\\ \texttt{msg\_dehom.\_macro\_recovery}};
  \node[outnode, below=of rs] (pf) {\texttt{iea\_s10\_elem\_plate\_forces.dat}\\ $\mathbf{F}_6 = \mathbf{A}_6\,\mathbf{s}_6$, 310 rows};
  \node[funcnode, below=of pf] (rp) {\texttt{msg\_rm\_plate.rm\_plate\_msg}\\ one through-thickness SG per\\ layup, built once at \texttt{fraction=frac}};
  \node[funcnode, below=of rp] (sd) {\texttt{msgrm\_strain\_at\_depth\_batch}\\ 3-D strain and stress at depth $z$,\\ \texttt{frame="material"} by default};
  \node[outnode, below=of sd] (ou) {\texttt{iea\_s10\_dehom\_v2.txt}\\ \texttt{iea\_s10\_dehom\_v2.npz}\\ \texttt{iea\_s10\_report.txt}\\ \texttt{iea\_s10\_v2\_section.png}};

  \draw[capedge] (bb) -- (mf); \draw[capedge] (mf) -- (rs);
  \draw[capedge] (rs) -- (pf); \draw[capedge] (pf) -- (rp);
  \draw[capedge] (rp) -- (sd); \draw[capedge] (sd) -- (ou);
}
...
This one stays a script: beyond the recovery itself it projects the result onto
the VABS gauss cloud, writes the validation report and draws four figures, none
of which the solver does. The bare recovery of a single station is the unified
command with the \texttt{D} argument,
\texttt{opensg <station>\_shell.yaml D}, which homogenizes and then recovers in
one run and takes its macro state from the \texttt{<station>\_shell.ff} file
next to the yaml rather than from a row of the sweep table.
