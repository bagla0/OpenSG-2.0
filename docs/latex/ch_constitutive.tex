...
\textbf{\texttt{name} is always the macro law's console title}, so the
\texttt{.out} header and the line the terminal prints are word-for-word the
same.
...
holds, and it is also the line the run prints to the terminal.}
\label{tab:outnames}
\scriptsize
\begin{tabular}{lll}
\toprule
...
\midrule
\texttt{"Cauchy Continuum"} & \texttt{The Effective Cauchy Continuum Stiffness Matrix} & 3-D solid $6\times6$ \\
\texttt{"Timoshenko Beam"} & \texttt{The Effective Timoshenko Beam Stiffness Matrix} & beam $6\times6$, shear-refined \\
\texttt{"Euler-Bernoulli Beam"} & \texttt{The Effective Euler-Bernoulli Beam Stiffness Matrix} & beam $4\times4$, classical \\
\texttt{"Classical Plate"} & \texttt{The Effective Classical Plate Stiffness Matrix} & plate ABD $6\times6$ \\
\texttt{"Reissner-Mindlin"} & \texttt{The Effective Reissner-Mindlin Stiffness Matrix} & plate ABDG $8\times8$ \\
\bottomrule
\end{tabular}
\end{table}

Only the first of these also prints
...
Two footnotes to the table. An empty \texttt{name} would give the anonymous
\texttt{The Effective Stiffness Matrix}, but no macro law uses it any more.
And the per-section \emph{wall} laws written by the other writer,
\texttt{opensg\_shell.sg\_homo.write\_abdg\_out}, keep the title
\texttt{The Effective Reissner-Mindlin Plate Stiffness Matrix}, one block per
section (Section~\ref{sec:wallabdg}) --- a wall plate law, not the macro law of
the run. Older documentation used \texttt{Timoshenko},
\texttt{Reissner-Mindlin Plate} and an anonymous
\texttt{The Effective Stiffness Matrix} for the 3-D solid law; those spellings
are stale, and the 25 baseline \texttt{.out} files under \texttt{examples/}
were regenerated to the titles above.
...
YAML's \texttt{msg:} key (or, if the key is absent, from the mesh dialect) and
runs the homogenization. The driver \texttt{1\_homo\_beam\_props.py} does the
same solve from Python and adds the figures. Its user block is three lines
long; only the station name changes:
...
title \texttt{The Effective Timoshenko Beam Stiffness Matrix}, and the per-section
...
The same dispatcher serves the solid engine: this YAML's header carries
\texttt{msg: solid} and \texttt{n\_model: 1}, so \texttt{opensg} hands it to
\texttt{opensg\_solid}. The driver \texttt{beam\_homo\_sg.py} runs the same
model from Python with the material table written out in full:
...
Timoshenko transformation. Which of the two lands in the \texttt{.out} is the
header's \texttt{refined:} key, exactly as on the plate side:
\texttt{refined: 1} writes the Timoshenko $6\times6$, and \texttt{refined: 0}
writes this $4\times4$ under the title
\texttt{The Effective Euler-Bernoulli Beam Stiffness Matrix}. Either way both
matrices are in the result dict, so a driver can print the other one, as
\texttt{beam\_homo\_sg.py} does.
...
which writes \texttt{RHC\_SW\_2UC\_45.out} with banner
...
\noindent
Which of the
two plate laws comes back is the header's \texttt{refined:} key:
\texttt{refined: 0} returns the classical ABD $6\times6$ of this section, under
the title \texttt{The Effective Classical Plate Stiffness Matrix}; the shipped
file sets \texttt{refined: 1}, so the run as shipped returns the
Reissner--Mindlin ABDG $8\times8$ of the next section, whose leading $6\times6$
is exactly the ABD quoted here. (\texttt{plate\_homo\_2dsg.py} is the same solve
from Python, and it also regenerates the \texttt{.yaml} and \texttt{.msh} from
the shipped \texttt{.sc}.) Here $\omega =
...
\texttt{The Effective Classical Plate Stiffness Matrix} at
\texttt{refined: 0}. Only the model choice
...
to \texttt{The Effective Reissner-Mindlin Stiffness Matrix} and its matrix to
the $8\times8$. Passing
...
\texttt{opensg\_shell.sg\_homo.shell\_sg3d} & 3-D shell SG (surface mesh) & $\mathbf D_{\mathrm{eff}}/\omega$, $\omega$ = bounding-box \emph{volume} \\
\bottomrule
\end{tabular}
\end{table}

For \texttt{build\_solid\_bundle} called directly from Python the normalising
area falls back to the convex hull of the contour
(\texttt{area\_source = "hull"}); pass \texttt{cell\_area} explicitly to use a
different measure, and the banner records which was used. The
\texttt{opensg <yaml>} CLI never takes that fallback: it supplies the measure
itself (\texttt{opensg\_shell.cli.sg\_cell\_area}) as the \texttt{omega:} header
key when the file declares one, and otherwise as the measured node
\emph{bounding box}, which is the periodic cell the assembly map ties and which
differs from the convex hull on any non-convex cell.
...
The last row of Table~\ref{tab:solidroutes} is worth stating plainly, because
for a three-dimensional SG the measure is a \textbf{volume}, not an area. The
$\omega$ of \texttt{shell\_sg3d} is the node bounding-box volume
...
periodic assembly map ties. It is the convention \texttt{opensg\_solid} uses
too, so the returned \texttt{C3D}, the line the CLI prints and
\texttt{<yaml>\_C3D.out} are one and the same matrix, and the printed moduli
compare directly against a solid SwiftComp \texttt{.K} file with no rescale.
An explicit \texttt{omega=} argument, or the YAML's \texttt{omega:} header key
which the CLI forwards, overrides the measure --- for instance with the
midsurface \emph{surface} area, which the result dict also returns as
\texttt{"surface\_area"}, if a per-unit-area wall law is what you want.
...
\item The transverse-shear block $\mathbf G$ is \textbf{always replaced} by the
  MSG least-squares result from \texttt{rm\_plate\_msg}, evaluated at the same
  \texttt{fraction}. It is not a shear-correction-factor estimate, and there is
  no switch that selects anything else. The former alternative
  \texttt{g\_source="whitney"} --- the complementary-energy shear flow --- has
  been retired: the keyword is still accepted by the old signatures, but
  passing it explicitly raises a \texttt{DeprecationWarning} and is ignored,
  and every emitted ABD cache records \texttt{g\_source: msg}. The
  complementary-energy $\mathbf G$ survives only as the internal numerical
  fallback on a laminate whose MSG $\mathbf U^*$-fit is not SPD.
\end{enumerate}
...
\midrule
Timoshenko beam $6\times6$ & \texttt{plate\_homo\_2d(n\_model=1)} $\rightarrow$ \texttt{\_beam\_homo\_kkt} & \texttt{<sg>.out} & \texttt{The Effective Timoshenko Beam Stiffness Matrix} \\
Timoshenko beam $6\times6$ & \texttt{build\_rm\_bundle} $\rightarrow$ \texttt{ring\_indep} & \texttt{<yaml>\_Timo.out} & \texttt{The Effective Timoshenko Beam Stiffness Matrix} \\
Timoshenko beam $6\times6$ & \texttt{segment\_timo\_from\_3dyaml} & \texttt{<yaml>\_Timo.out} & \texttt{The Effective Timoshenko Beam Stiffness Matrix} \\
Euler--Bernoulli beam $4\times4$ & \texttt{\_beam\_homo\_kkt} $\rightarrow$ \texttt{r["C\_eff\_EB"]} (\texttt{refined: 0}) & \texttt{<sg>.out} & \texttt{The Effective Euler-Bernoulli Beam Stiffness Matrix} \\
...
RM plate ABDG $8\times8$ & \texttt{plate\_homo\_2d(n\_model=2, shear\_refined=True)} $\rightarrow$ \texttt{plate\_shear\_ladder} & \texttt{<sg>.out} & \texttt{The Effective Reissner-Mindlin Stiffness Matrix} \\
RM plate ABDG $8\times8$ & \texttt{rm\_plate\_msg} $\rightarrow$ \texttt{r["ABDG"]} & \texttt{<db>\_plate\_homo.out} & \texttt{The Effective Reissner-Mindlin Stiffness Matrix} \\
3-D solid $6\times6$ & \texttt{plate\_homo\_2d(n\_model=3)} & \texttt{<sg>.out} & \texttt{The Effective Cauchy Continuum Stiffness Matrix} + engineering constants \\
3-D solid $6\times6$ & \texttt{build\_solid\_bundle} $\rightarrow$ \texttt{ring\_solid} & \texttt{<yaml>\_C3D.out} & \texttt{The Effective Cauchy Continuum Stiffness Matrix} + engineering constants \\
3-D solid $6\times6$ & \texttt{sg\_homo.shell\_sg3d} & \texttt{<yaml>\_C3D.out} & \texttt{The Effective Cauchy Continuum Stiffness Matrix} + engineering constants \\
...
\paragraph{The files the \texttt{D} route writes.} Run from the CLI as
\texttt{opensg <yaml> D}, the chain above writes
\texttt{<base>\_dehom.txt} (one row per recovery point: 15 field columns, the
six stresses, six strains and three displacements), the matching
\texttt{<base>\_dehom.vtk}, and --- unless the header sets
\texttt{junction: off} --- the \texttt{<base>\_dehom.junc} sidecar. At the
default \texttt{junction: flag} tier the \texttt{.txt} carries two \emph{extra}
columns at the very end, \texttt{jflag} and \texttt{jdist}, which are also
written as extra \texttt{CELL\_DATA} scalars in the \texttt{.vtk}: they flag the
recovery stations that fall inside a wall-junction boundary layer, where two
crossing walls hang their through-thickness columns over the same material.
Nothing else changes at \texttt{flag}. The full treatment, including the
\texttt{exclude} tier and the \texttt{junction\_bl} and \texttt{junction\_ang}
keys, is Section~\ref{sec:shell-dehom-junction}.
