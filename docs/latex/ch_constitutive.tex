...
For the IEA-22 blade station at $r/R = 0.20$ that $6\times6$ has the diagonal
...
$[\,9.8993\times10^{6},\;1.9075\times10^{6},\;9.3619\times10^{5},\;
2.9775\times10^{8},\;2.1896\times10^{9},\;9.2899\times10^{8}\,]$ in the input
units (N and N$\cdot$mm$^2$, since the SG is in millimetres and the moduli in
MPa).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.72\textwidth]{\FIGDIR/rhc_2dsg_mesh.png}
\caption{The two-dimensional honeycomb SG \texttt{RHC\_SW\_2UC\_45}, the same
mesh used by the beam ($n_{\mathrm{model}}=1$), plate ($2$) and solid ($3$)
examples. Colours are the three material sets of the \texttt{.yaml}: the
$+45$ and $-45$ composite plies of the cell walls and the isotropic
face sheets.}
\label{fig:rhcmesh}
\end{figure}
...
with $\boldsymbol\varepsilon$ and $\boldsymbol\kappa$ exactly the first and
second triples of Eq.~\eqref{eq:abd}.
...
(\texttt{fraction: 0.5}) marked. This mesh is the SG whose solution is the
$8\times8$ of Eq.~\eqref{eq:abdg}.}
\label{fig:plate1dsg}
\end{figure}
...
fires). The banner of the \texttt{.out} reports all three counts.}
\label{fig:schwarz}
\end{figure}

\subsection{A worked solid law: the TPMS Sample\_1 cell}

The reference case for Eq.~\eqref{eq:c3d} on the solid engine is
...
that solves Eq.~\eqref{eq:c3d} under generalized plane strain with periodic
...
\caption{The UD fibre/matrix two-dimensional SG of
\texttt{examples/OpenSG-solid/5\_get\_solid\_props\_from\_SG}, whose equivalent
3-D law reproduces the published nine constants to $0.002\,\%$.}
\label{fig:udcomp}
\end{figure}
...
[\mathbf 0,\mathbf 0,\mathbf G]]$ as Eq.~\eqref{eq:abdg}, with the shell
...
Eq.~\eqref{eq:timo}, $\mathbf{st} = \mathbf C_6^{-1}\,\mathbf{FF}$, with
...
$\mathbf{FF} = [\,4.1927\times10^{4},\;8.2674\times10^{3},\;
1.4673\times10^{6},\;1.2191\times10^{6},\;-6.0185\times10^{7},\;
3.0426\times10^{5}\,]$ in N and N$\cdot$m, i.e.\ a flapwise-moment-dominated
state. \texttt{\_macro\_fields} recombines the ring warping modes $V_0$ and
...
These are exactly the resultants of Eq.~\eqref{eq:abd}: the beam model has
handed each wall a plate problem. They are written to
\texttt{<name>\_elem\_plate\_forces.dat}, one row per ring element, with columns
...
Eq.~\eqref{eq:c3d} for \texttt{n\_model=3}, or the six Timoshenko beam strains
of Eq.~\eqref{eq:timo} for \texttt{n\_model=1}. The beam case is run by
