...
laminate shorthand and one legacy SwiftComp converter; a second converter turns
a gmsh surface mesh into the shell dialect
(Section~\ref{sec:input-mshconv}). Which dialect you write is decided
...
\texttt{<Name>} infix is \emph{always} the macro law's own console title, so
the file and the terminal read the same; the five names are listed in
Table~\ref{tab:input-outnames}. Only the 3-D solid law is written with
\texttt{constants=True} and therefore also prints
...
\centering
\small
\caption{The five \texttt{<Name>} infixes \texttt{write\_sc\_K} emits. The name
is the macro law's console title, so the terminal and the \texttt{.out} agree
word for word.}
\label{tab:input-outnames}
\begin{tabular}{@{}ll@{}}
\toprule
macro law & \texttt{<Name>} \\
\midrule
3-D solid (Cauchy continuum) $6\times6$ & \texttt{Cauchy Continuum} \\
beam $6\times6$, shear-refined & \texttt{Timoshenko Beam} \\
beam $4\times4$, classical & \texttt{Euler-Bernoulli Beam} \\
plate ABD $6\times6$, classical & \texttt{Classical Plate} \\
plate ABDG $8\times8$, shear-refined & \texttt{Reissner-Mindlin} \\
\bottomrule
\end{tabular}
\end{table}

The per-section \emph{wall} plate laws are a different writer,
\texttt{opensg\_shell.sg\_homo.write\_abdg\_out}, and keep the title
\texttt{The Effective Reissner-Mindlin Plate Stiffness Matrix} --- one
$8\times8$ block per section in \texttt{<base>\_ABDG.out}.
...
\begin{lstlisting}[style=shellst,caption={One orientation row: e1 = [0,0,1] out of plane, then the in-plane e2 and e3.},basicstyle=\ttfamily\footnotesize]
elementOrientations:
...
The file is the whole problem: its header carries \texttt{n\_model: 1},
\texttt{refined: 1} and \texttt{msg: shell}, so that one command picks the
msg-shell engine and the Timoshenko route without a flag, and writes
\texttt{iea\_s10\_shell\_Timo.out} and \texttt{iea\_s10\_shell\_ABDG.out}. The
shipped driver \texttt{beam\_homo\_shell.py} makes the same call through the
API and adds the two figures the CLI does not draw. It sets
\texttt{name = "iea\_s10"}, calls
\texttt{build\_rm\_bundle(name + "\_shell.yaml")}, and prints the $6\times6$
with the diagonal labelled \texttt{EA GA2 GA3 GJ EI2 EI3}:

\begin{lstlisting}[style=opensg,caption={The whole compute part of beam\_homo\_shell.py.},basicstyle=\ttfamily\footnotesize]
...
the footer records \texttt{ Time taken: 2.58 sec}.
...
Two files come back, not one. \texttt{<base>\_C3D.out} carries the equivalent
solid law, and \texttt{<base>\_ABDG.out} carries the \emph{step-1} record that
produced it: the $8\times8$ Reissner--Mindlin wall plate law and its
compliance, one block per section, written by the same
\texttt{write\_abdg\_out} the cross-section routes use. Every route that
reduces a layup to a wall plate law now emits it --- the cross-section rings
(beam \texttt{refined: 1} and the \texttt{n\_model: 3} equivalent solid) and
the 3-D shell SG alike --- so the wall law is always auditable next to the
answer it fed.
...
with $E = 69$~GPa and $\nu = 0.3$. The surface may hold triangles and quads at
once: they are assembled as two \emph{separate} element batches scattered into
the same global system, never padded into one another (the old collapsed quad
\texttt{[n1 n2 n3 n3]} is rejected by \texttt{check\_element\_edges}, because
its zero-length edge made the MITC tying point singular). Ids are 1-based as
in the 1-D dialect. The mesh itself is shown in
Figure~\ref{fig:input-schwarz-mesh}.
...
python make_schwarz_yaml.py          # writes schwarz_p_3Dshell.yaml
opensg schwarz_p_3Dshell.yaml        # homogenizes it
\end{lstlisting}

The first command is the mesh generator, which has no command-line
equivalent; the second is the plain homogenization, and the YAML's header
(\texttt{n\_model: 3}, \texttt{msg: shell}) describes it completely. It writes
\texttt{schwarz\_p\_3Dshell\_C3D.out}, whose banner records the mesh size and
element mix, the junction-edge count, the boundary treatment and the
...
elems [26360 tri3/MITC3], 0 junction edges, periodic in 3 dirs, per unit cell
1)}. Because the 3-D solid law is written with \texttt{constants=True}, the
file ends with the orthotropic-approximated engineering constants --- here
$E_1 = 9.2745\times10^{8}$, $E_2 = 9.2749\times10^{8}$,
$E_3 = 9.2712\times10^{8}$~Pa and $G_{12} = 1.0395\times10^{9}$~Pa --- followed
by \texttt{ Time taken: 10.85 sec}. The near-equality of the three Young's
moduli is the physical check: a Schwarz-P cell is cubic-symmetric, so any
appreciable spread means the mesh or the frames are wrong. The companion
\texttt{schwarz\_p\_3Dshell\_ABDG.out} holds the single wall law that fed it:
$A_{11} = 2.7643220\times10^{9}$, $A_{12} = 8.2929659\times10^{8}$,
$A_{66} = 9.6751264\times10^{8}$, $D_{11} = 3.0617465\times10^{5}$ and the
transverse shear $G = 8.5193106\times10^{8}$~N/m. The driver
\texttt{schwarz\_solid\_props.py} makes the same \texttt{shell\_sg3d} call and
additionally writes \texttt{schwarz\_p\_mesh.png}.
...
\emph{surface}, so it carries no layups and no orientations --- each material
block is already a $6\times6$ stiffness or a set of engineering constants with
a ply angle. It is the \textbf{native user input}: you author it directly, and
a SwiftComp \texttt{.sc} is only one optional way to generate one.
...
excerpt below keeps the analysis header, two rows per list and two of the three
material blocks; the header keys are collected in
Table~\ref{tab:input-header-keys}, the mesh keys in
Table~\ref{tab:input-solid-keys} and the material types in
Table~\ref{tab:input-mattypes}.

\begin{lstlisting}[style=shellst,caption={Abridged RHC\_SW\_2UC\_45.yaml. The leading scalar keys are the analysis request; the mesh blocks follow.}]
n_model: 2      # 1 = beam, 2 = plate, 3 = solid -- the macro model
refined: 1      # 0 = classical (plate ABD / beam EB); 1 = shear-refined
msg: solid      # the ENGINE that owns the file; `opensg <yaml>` dispatches on it
nodes:
...
\subsection{The header contract}
\label{sec:input-header}

The header is the leading block of scalar keys above the first mesh block. It
is the \emph{analysis request} --- what to run, not what the SG is --- and it
is shared by both engines, so \texttt{opensg}, \texttt{opensg\_solid} and
\texttt{opensg\_shell} all read the same keys.
\textbf{Every key is defaulted}, and a headerless mesh still runs (as a
classical plate homogenization).

\begin{table}[htbp]
\centering
\small
\caption{The YAML header contract. Every key is optional; the last three are
read by the msg-shell \texttt{D} route only.}
\label{tab:input-header-keys}
\begin{tabular}{@{}llp{7.3cm}@{}}
\toprule
key & values & meaning \\
\midrule
\texttt{msg} & \texttt{shell} $|$ \texttt{solid}
  & which engine owns the file; omit it and the mesh dialect decides \\
\texttt{n\_model} & \texttt{1} $|$ \texttt{2} $|$ \texttt{3}
  & the macro model: 1 beam, 2 plate, 3 3-D solid \\
\texttt{refined} & \texttt{0} $|$ \texttt{1}
  & 0 classical (plate ABD $6\times6$ / beam EB $4\times4$), 1 shear-refined
    (plate ABDG $8\times8$ / beam Timoshenko $6\times6$); ignored for the
    solid macro model \\
\texttt{analysis} & \texttt{H} $|$ \texttt{D}
  & homogenization or dehomogenization; the command-line argument wins \\
\texttt{epsilon\_bar} & 6 floats & the macro state a \texttt{D} run recovers from \\
\texttt{omega} & float
  & a user SG measure, overriding the one measured from the mesh \\
\texttt{aperiodic} & \texttt{1}
  & zero fluctuation on every bounding-box-face node instead of periodicity;
    omit the key for the periodic default \\
\texttt{junction} & \texttt{off} $|$ \texttt{flag} $|$ \texttt{exclude}
  & msg-shell \texttt{D} only: the junction-aware recovery tier \\
\texttt{junction\_bl} & float
  & msg-shell \texttt{D} only: boundary-layer radius in wall thicknesses
    (default 1.0) \\
\texttt{junction\_ang} & float (deg)
  & msg-shell \texttt{D} only: tangent-grouping tolerance of the junction
    detector (default 1.0) \\
\bottomrule
\end{tabular}
\end{table}

\noindent
\textbf{There is no \texttt{dim:} key and no \texttt{scale:} key.} The SG
dimension is read from the mesh --- the element node count, with ties broken
by the leading node-coordinate columns that actually vary --- and the SG
measure $\omega$ is measured from the mesh as well. A legacy \texttt{dim:} or
\texttt{scale:} left in an old file is still tolerated by the loader, but
neither is part of the input contract and neither should be written into a new
file.

\begin{table}[htbp]
\centering
\small
\caption{Mesh keys of the solid SG YAML, as consumed by
...
\texttt{nodes} & comma-separated coordinate rows, always three components.
  Only the leading \texttt{n\_sg} columns are used
  (\texttt{points = nodes[:, 0:n\_sg]}), so a column that never varies --- the
  trailing zeros above --- is padding. \\
...
Any block may additionally carry \texttt{angle} (degrees --- the in-plane ply
rotation, applied when the material is built from constants, and the way a
$\pm45$ laminate lives in the file) and \texttt{density} (used by the beam
route's mass $6\times6$). \texttt{aux} is the auxiliary line carried through
verbatim when a file was converted from an \texttt{.sc}; the elastic path does
not consume it, and a hand-written YAML simply omits it.
...
The recommended pattern keeps the ply data in exactly one place --- the YAML
itself: \texttt{type: 1} blocks carrying the nine engineering constants plus
their \texttt{angle}, and the analysis request in the header. Nothing then has
to be passed at the call site, and the run is one command:
...
r = plate_homo_2d(name + ".yaml")     # header + materials come from the file
print(r["law_title"] + ":")
print(r["law"])
\end{lstlisting}

\noindent
(The \texttt{material\_param=} / \texttt{angles=} override arguments remain for
scripting over pre-rotated \texttt{type: 2} files, as examples 4 and 7 do.) The
header's \texttt{n\_model} picks the macro model --- 1 beam, 2 plate, 3
equivalent 3-D solid --- and \texttt{refined} upgrades the plate ABD to the
ABDG, or the beam EB to Timoshenko. This file sets \texttt{refined: 1}, so the
run writes \texttt{RHC\_SW\_2UC\_45.out} ---
\texttt{The Effective Reissner-Mindlin Stiffness Matrix}, rows
\texttt{[N11 N22 N12 M11 M22 M12 Q1 Q2]}, under the banner
...
in-plane span measured from the mesh: the YAML carries no normalization key,
because the homogenizer always measures its own SG. For a cell converted from
SwiftComp that measurement agrees with the \texttt{.sc} trailing line
(\texttt{35.248} here) to the digits that line carries, and that agreement is
the check that the mesh and the \texttt{.sc} header describe the same cell. For
this run $A_{11} = 3.3586\times10^{5}$, $D_{11} = 9.0368\times10^{7}$ in the
file's own units, and the footer reads \texttt{ Time taken: 1.89 sec}.
...
\texttt{opensg\_solid.helper.sc\_to\_yaml} module docstring, implemented in its
\texttt{read\_sc}, and summarized in Table~\ref{tab:input-sc}. (The historical
\texttt{opensg\_solid.rm\_plate\_1D.helper.sc\_to\_yaml} path still imports:
it is now a thin shim that re-exports the module above. New code should use the
canonical path.)
...
trailing line & the \texttt{scale} (volume or thickness normalization). It is
  read and kept on the parsed dict for the cross-check above, but it is
  \emph{not} written into the YAML as a \texttt{scale:} key --- $\omega$ is
  measured from the mesh. The converter re-emits it as the optional
  \texttt{omega:} header key in the one case the solver would consume it, a
  3-D SG homogenized to a 3-D solid, and then only when it differs from the
  measured bounding-box volume by more than $10^{-6}$ relative \\
\bottomrule
\end{tabular}
\end{table}
...
\subsection{From a gmsh surface mesh: \texttt{msh\_to\_yaml}}
\label{sec:input-mshconv}

The shell twin of the \texttt{.sc} converter is
\texttt{opensg\_shell.helper.msh\_to\_yaml}, which turns an ASCII gmsh
\texttt{.msh} (format 2.2 or 4.1) \emph{surface} mesh into an msg-shell SG
YAML. A gmsh mesh knows the geometry and nothing else, so the converter writes
the whole mesh side and never invents the constitutive side. It emits the
header, \texttt{nodes}, \texttt{elements}, one \texttt{elementOrientations}
row per facet, and one \texttt{layup\_<k>} element set per gmsh physical tag.
The per-facet frame is the one \texttt{make\_schwarz\_yaml.py} established:
$e_3$ is the facet normal, $e_2$ the first edge, and $e_1 = e_2 \times e_3$,
right-handed. Only 3-node triangles (gmsh type 2) and 4-node quads (type 3)
are accepted; the 0- and 1-D entities gmsh writes for physical points and
curves are skipped, and any other element type is an error.

What you get depends on how much of the layup you supply
(Table~\ref{tab:input-mshconv}).

\begin{table}[htbp]
\centering
\small
\caption{The three calling forms of
\texttt{opensg\_shell.helper.msh\_to\_yaml.convert}.}
\label{tab:input-mshconv}
\begin{tabular}{@{}p{4.4cm}p{7.4cm}@{}}
\toprule
what you pass & result \\
\midrule
nothing (the default) & a marked \texttt{FILL\_IN} \textbf{template}:
  the mesh is complete, \texttt{sections:} and \texttt{materials:} are
  placeholders. \texttt{opensg\_shell} refuses to run it, and
  \texttt{check\_filled} names the fields still unfilled \\
\texttt{thickness=t} & a runnable single-ply YAML; the
  default material is aluminium, $E = 69$~GPa, $\nu = 0.30$, density
  $2700$~kg/m$^3$ \\
\texttt{layup=[[mat, t, angle], ...]} \newline plus \texttt{materials=[...]} &
  the general multi-ply form \\
\bottomrule
\end{tabular}
\end{table}

\begin{lstlisting}[style=opensg,caption={Converting a gmsh surface mesh to an msg-shell SG YAML.}]
from opensg_shell.helper.msh_to_yaml import convert

convert("schwarz_p_D2_shell.msh")                      # FILL_IN template
convert("schwarz_p_D2_shell.msh", thickness=0.036457)  # runnable, aluminium
\end{lstlisting}

\noindent
The \texttt{sections:} and \texttt{materials:} blocks are written
\textbf{first}, above the mesh, because they are the part a human edits: a TPMS
cell runs to $10^4$ nodes and $10^5$ mesh lines, and a \texttt{sections:} key
buried at line 66262 of a 4~MB file looks missing. A YAML mapping is unordered
by specification, so every reader in the stack is indifferent to the order.
...
\texttt{model} & \texttt{0} prints the classical $6\times6$ ABD, \texttt{1} the
  ...
  \texttt{.out}: \texttt{Classical Plate} or \texttt{Reissner-Mindlin}. \\
...
The 3-D shell SG uses the same quad element for its quads, and a genuine
3-node triangle (MITC3) for its triangles: the two are assembled as separate
element batches into one global system, and the old collapsed quad
\texttt{[n1 n2 n3 n3]} is rejected rather than accepted.
...
$y_3$ of the solid Voigt order above. For an SG of dimension 2 that is node
coordinate index 1, since only the leading two coordinates are read; for a
