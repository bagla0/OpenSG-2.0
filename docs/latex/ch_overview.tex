...
msg-shell solid props, cross-section SG & \texttt{build\_solid\_bundle} & \texttt{<yaml>\_C3D.out} \\
msg-shell 3-D shell SG       & \texttt{shell\_sg3d} & \texttt{<yaml>\_C3D.out} \\
\bottomrule
\end{tabular}
\end{table}
...
\captree{
  ...
  \node[outnode, right=5mm of f3] (o3) {\texttt{schwarz\_p\_3Dshell\_C3D.out}\\ the equivalent solid $6\times6$,\\ per unit-cell volume};
  ...
  \node[outnode, right=5mm of f4] (o4) {\texttt{RHC\_SW\_2UC\_45.out}\\ \texttt{n\_model} 1 named Timoshenko;\\ 2 Classical Plate (or the\\ Reissner-Mindlin one if\\ \texttt{shear\_refined}); 3 the 3-D law};
...
\caption{The four pipelines, one row each: the three msg-shell routes and the
         ...
         a single layout; the \texttt{\_ABDG.out} companion of the ring route
         is the one exception, written by \texttt{write\_abdg\_out}.}
\label{fig:pipelines}
\end{figure}
...
g\_source="msg")}, and it proceeds as follows.
...
\item \texttt{sg\_materials.\_material\_by\_section} builds, per section, the
      ...
      transverse-shear $G$.
\item With the default \texttt{g\_source="msg"}, the wall $G$ of every section
      is replaced by \texttt{rm\_plate\_msg}, the MSG (Yu-2002 least-squares)
      through-thickness solve in \texttt{opensg\_solid.rm\_plate\_1D}, run at
      the same \texttt{fraction=frac}. The alternative is
      \texttt{g\_source="whitney"}, the complementary-energy shear-flow value.
      The per-section laws are written to \texttt{<yaml>\_ABDG.out}.
...
\noindent
It writes \texttt{iea\_s10\_shell\_Timo.out} (the Timoshenko stiffness and
compliance), \texttt{iea\_s10\_shell\_ABDG.out} (the per-section wall laws),
\texttt{iea\_s10\_mesh.png} (the contour coloured by layup) and
\texttt{iea\_s10\_orient.png} (the compulsory $e_1/e_2/e_3$ orientation figure,
emitted by \texttt{auto\_emit}). The printed diagonal is
...
g\_source="msg", boundary="periodic")}.
...
\item The YAML's nodes, elements and \texttt{elementOrientations} are read; the
      ...
      Triangles are accepted by repeating the last node.
\item The single wall law (ABD at the mid-surface reference plus the $2\times2$
      $G$, again from \texttt{rm\_plate\_msg} when
      \texttt{g\_source="msg"}) is built once and applied to every element.
...
\item The boundary treatment. With the default \texttt{boundary="periodic"} the
...
\item $\mathbf{D}_{\mathrm{eff}} = \mathbf{D}_{ee} +
      ...
      The default $\omega$ is the integrated midsurface \emph{surface area} ---
      the 3-D analogue of the plane-section $\omega = $ perimeter --- and that
      is the normalisation of the returned \texttt{C3D}. The
      \texttt{<yaml>\_C3D.out} file is instead written per unit-cell
      \emph{volume}, so its moduli compare directly with a solid \texttt{.K}
      file.
\end{enumerate}
...
\noindent
The run prints the junction-edge count, the number of DOF and the solve time,
then the $6\times6$; it writes \texttt{schwarz\_p\_3Dshell\_C3D.out} and
\texttt{schwarz\_p\_mesh.png}. The companion script
...
macro strain set $\Gamma_e$ is built. The steps shared by all three are:
...
\item \texttt{sg\_mesh.load\_sg\_input} parses the \texttt{.sc} or YAML into a
      ...
      \texttt{cells}, \texttt{mat\_id} and the materials. With
...
\item[\texttt{n\_model=1}, beam.] Control goes to \texttt{\_beam\_homo\_kkt}:
      ...
      output is \texttt{<base>.out}, named \emph{Timoshenko}, with $\omega$ in
      the banner. This path needs an SG of dimension $\ge2$.
\item[\texttt{n\_model=2}, plate.] The macro strain set is
      ...
      written as \emph{Classical Plate}. With \texttt{shear\_refined=True} the
      first-order warping ladder \texttt{plate\_shear\_ladder} runs as well and
      the result gains \texttt{G\_msg} ($2\times2$), \texttt{ABDG} (the
      $8\times8$ with the shear block on the diagonal) and
      \texttt{A6\_ladder}.
\item[\texttt{n\_model=3}, solid.] The macro strain set is the six 3-D strains
      and the answer is the equivalent solid $6\times6$, written with the
