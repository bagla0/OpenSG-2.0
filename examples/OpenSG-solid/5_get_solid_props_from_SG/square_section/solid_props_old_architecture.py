...
OpenSG OLD architecture: the original single-script JAX_BICGoptimize driver
(n_model = 3), unchanged apart from its User Input block and an output section.

Reads square_tube.sc (the same mesh as square_tube_2Dsolid.yaml, re-emitted in
SwiftComp form).  A .sc carries one material id per element and this driver
applies one fibre angle per material, so per-element material frames cannot be
represented -- the ply is left unrotated, which is exactly the setting the new
architecture is compared against in README.md.

Run:  python solid_props_old_architecture.py
      (needs the fe_jax shipped with ~/OpenSG_2.0, put on sys.path below, and a
       jax new enough for its jit decorators -- e.g. envs/jax_env)
"""
import sys, os
sys.path.insert(0, os.path.expanduser("~/OpenSG_2.0"))
sys.path.insert(0, os.path.expanduser("~/OpenSG_2.0/msg_akshat"))
...
name='square_tube'   # reads <name>.sc (square tube 2-D SG, midline a=1, t=0.03)

# m45 ply, SI (Pa): E1 E2 E3 G12 G13 G23 v12 v13 v23
material_param=jnp.array([(142.0e9, 9.8e9, 9.8e9, 6.0e9, 6.0e9, 4.8e9,
                          0.30, 0.30, 0.42)])

angles = jnp.array([0.0]) # one angle per material domain; the .sc cannot carry
                          # per-element frames, so the ply is left unrotated
#####################################################
...
fe_type = FiniteElementType(
    ...
    quadrature_type=getattr(QuadratureType, "default",
                            getattr(QuadratureType, "Default", None)),
...
with open("square_solid_old_architecture.out", "w") as _f:
    _f.write("# equivalent 3-D solid properties, OLD JAX_BICGoptimize single"
             " script (n_model=3)\n")
    _f.write("# source: square_tube.sc (square tube, midline a=1.0, t=0.03,"
             " m45 ply, angles=0)\n")
    ...
    for _k, _v in _rows:
        _f.write("%-5s %16.8e\n" % (_k, _v))
print("wrote square_solid_old_architecture.out")
