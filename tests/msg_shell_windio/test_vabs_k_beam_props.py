...
CASES = [
    (0.2, "iea_s10.sg.K", [0.05, 0.12, 0.12, 0.08, 0.06, 0.06]),
    (0.5, "iea_s25.sg.K", [0.05, 0.12, 0.12, 0.08, 0.06, 0.06]),
    (0.7, "iea_s35.sg.K", [0.05, 0.12, 0.12, 0.08, 0.06, 0.06]),
]
...
def test_beam_props_vs_vabs(blade, tmp_path, r, kref, tol):
    ...
    # Timoshenko diagonals vs the VABS 2-D solid
    for i in range(6):
    ...
    assert abs(M6[0, 0] / Mv[0, 0] - 1.0) < 0.03, \
    ...
    for (i, j, lab) in [(3, 3, "i11"), (4, 4, "i22"), (5, 5, "i33")]:
        ...
        assert rel < 0.06, "mass %s at r=%.1f: %.5g vs VABS %.5g (%.2f%%)" % (
