...
Output order everywhere: SwiftComp (xx, yy, zz, yz, xz, xy).
...
#   coords              (E*Q, d) Gauss coordinates (gauss_coords)
...
def compute_fluctuations_gpu(Deff_srt, st, V0, V1):
    """Successive-derivative Timoshenko recovery (Beam_solid.py): from
    the applied macro state st [x1, shear2, shear3, twist, bend2, bend3]
    build the three derivative levels and the fluctuation displacement
    vectors the Gauss recovery consumes."""
...
def plate_dehom_2d(r: Dict[str, Any],
    ...
    if r["n_model"] == 1:
        ...
        return np.asarray(Gam), np.asarray(Sig)
...
def export_gauss(r, Gamma_eqs, Sigma_eqs, prefix="gauss_results"):
    ...
    (verbatim export of the SSDM script, parameterized)."""
    ...
    print("export_gauss: %d points -> %s.txt / %s.vtk"
          % (num_pts, prefix, prefix))
